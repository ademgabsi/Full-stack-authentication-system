...
import { EmailVerificationToken as EVEntity } from './entities/email-verification-token.entity';
...
const mockEvRepo = createMockRepo();
const mockRtRepo = createMockRepo();
const mockDfRepo = createMockRepo();
const mockAlRepo = createMockRepo();
const mockScRepo = createMockRepo();
const mockWaRepo = createMockRepo();
const mockPrRepo = createMockRepo();
...
const mockMfa = {
  ...
  verifyTotp: jest.fn((_secret?: string, _token?: string) => true as boolean),
...
const mockDeviceFingerprint = {
  ...
  getOrCreateFingerprint: jest.fn(() => Promise.resolve({ isNew: false, fingerprint: { id: 'fp-1' } })),
};
const mockAnomaly = {
...
function createAuthServiceProvider() {
  return {
    provide: AuthService,
    useFactory: (
      userR: any, rtR: any, prR: any, evR: any, ds: any,
      jwt: any, config: any, email: any, mfa: any, audit: any,
      captcha: any, breach: any, wh: any, df: any, anomaly: any, stepUp: any,
    ) => new AuthService(
      userR, rtR, prR, evR, ds, jwt, config, email, mfa, audit, captcha, breach, wh, df, anomaly, stepUp,
    ),
    inject: [
...
describe('Security Tests', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: JwtService, useValue: { sign: jest.fn(() => 'jwt-token'), verify: jest.fn() } },
  ...
  describe('Anti-Enumeration (User Existence)', () => {
    it('should return same message for registered and non-registered emails during registration', async () => {
      ...
      const existingResult = await authService.register({
        email: 'existing@example.com', password: 'StrongPass1!', fullName: 'User',
      });

      mockUserRepo.findOne.mockResolvedValue(null);
      const newResult = await authService.register({
        email: 'new@example.com', password: 'StrongPass1!', fullName: 'User',
      });
    ...
    it('should not reveal whether email is registered during login', async () => {
      mockUserRepo.findOne.mockResolvedValue(null);
      await expect(authService.login({ email: 'nonexistent@example.com', password: 'Pass1!' })).rejects.toThrow('Invalid credentials');

      const pwHash = await bcrypt.hash('Pass1!', 10);
      mockUserRepo.findOne.mockResolvedValue(buildUser({ isVerified: false, passwordHash: pwHash }));
    ...
    it('should not reveal whether email is registered during forgot-password', async () => {
      mockUserRepo.findOne.mockResolvedValue(null);
      const result1 = await authService.forgotPassword({ email: 'nonexistent@example.com' });

      mockUserRepo.findOne.mockResolvedValue(buildUser());
      const result2 = await authService.forgotPassword({ email: 'test@example.com' });
    ...
    it('should return generic error for unverified users', async () => {
      const pwHash = await bcrypt.hash('Pass1!', 10);
      mockUserRepo.findOne.mockResolvedValue(buildUser({ isVerified: false, passwordHash: pwHash }));
      await expect(authService.login({ email: 'test@example.com', password: 'Pass1!' })).rejects.toThrow('Invalid credentials');
    });
  });
  ...
  describe('Brute Force Protection', () => {
    it('should increment failed attempts on wrong password', async () => {
      const pwHash = await bcrypt.hash('Pass1!', 10);
      mockUserRepo.findOne.mockResolvedValue(buildUser({ isVerified: true, passwordHash: pwHash }));
      await expect(authService.login({ email: 'test@example.com', password: 'Wrong1!' })).rejects.toThrow('Invalid credentials');
    ...
    it('should lock account after max failed attempts', async () => {
      ...
      mockUserRepo.findOne
        .mockResolvedValueOnce(buildUser({ isVerified: true, passwordHash: pwHash, failedAttempts: 4 }))
        .mockResolvedValueOnce(buildUser({ isVerified: true, passwordHash: pwHash, failedAttempts: 5 }));
  ...
  describe('Password Security', () => {
    it('should reject password from data breaches during registration', async () => {
      ...
      await expect(authService.register({
        email: 'test@example.com', password: 'Breached1!', fullName: 'User',
    ...
    it('should reject password from data breaches during password reset', async () => {
      ...
      mockUserRepo.findOne.mockResolvedValue(buildUser({ passwordHash: oldPwHash }));
      mockPasswordResetRepo.find.mockResolvedValue([{
        id: 'pr-1', code: hashedCode, userId: 'user-1',
        expiresAt: new Date(Date.now() + 100000), used: false,
      }]);
      mockBreach.isBreached.mockResolvedValue(50);

      await expect(authService.resetPassword({
        email: 'test@example.com', code, password: 'LeakedPass1!',
      })).rejects.toThrow('data breaches');
    });
    ...
    it('should reject reuse of same password during reset', async () => {
      ...
      mockUserRepo.findOne.mockResolvedValue(buildUser({ passwordHash: samePwHash }));
      mockPasswordResetRepo.find.mockResolvedValue([{
        id: 'pr-1', code: hashedCode, userId: 'user-1',
        expiresAt: new Date(Date.now() + 100000), used: false,
      }]);
      ...
      await expect(authService.resetPassword({
        email: 'test@example.com', code, password: 'SamePass1!',
    ...
    it('should hash password with sufficient cost factor', async () => {
      mockUserRepo.findOne.mockResolvedValue(null);
      mockBreach.isBreached.mockResolvedValue(0);
      const result = await authService.register({ email: 'test@example.com', password: 'StrongPass1!', fullName: 'User' });
  ...
  describe('Token Security & Replay Detection', () => {
    it('should revoke all tokens in family on replay attack', async () => {
      const fakeTxManager = {
        createQueryBuilder: jest.fn().mockReturnValue({
          ...
          getOne: jest.fn().mockResolvedValue({
            id: 'rt-1', token: 'hashed', userId: 'user-1', family: 'family-1',
            isRevoked: true, expiresAt: new Date(Date.now() + 10000000),
      ...
      mockDataSource.transaction.mockImplementation(async (_level: any, cb: any) => cb(fakeTxManager));

      await expect(authService.refreshTokens('replayed-token')).rejects.toThrow('revoked');
    ...
    it('should hash refresh tokens before storage', async () => {
      ...
      mockRefreshTokenRepo.findOne.mockResolvedValue({ id: 'rt-1', token: 'hashed-value', userId: 'user-1', isRevoked: false });
      await authService.logout('raw-refresh-token', 'user-1');
      expect(mockRefreshTokenRepo.findOne).toHaveBeenCalledWith(
        expect.objectContaining({ where: expect.objectContaining({ token: expect.any(String) }) }),
      );
    });
  });
  ...
  describe('MFA Security', () => {
    it('should require password + MFA to disable MFA', async () => {
      ...
      mockUserRepo.findOne.mockResolvedValue(buildUser({
        mfaEnabled: true, mfaSecret: 'SECRET', passwordHash: pwHash,
      }));
      mockMfa.verifyTotp.mockReturnValue(true);

      const result = await authService.disableMfa('user-1', { password: 'StrongPass1!', totpCode: '123456' });
    ...
    it('should reject MFA disable without TOTP code', async () => {
      ...
      await expect(authService.disableMfa('user-1', { password: 'StrongPass1!' })).rejects.toThrow('Current MFA code is required');
    });

    it('should reject invalid TOTP codes', async () => {
      mockMfa.verifyTotp.mockReturnValue(false);
      mockUserRepo.findOne.mockResolvedValue(buildUser({ mfaSecret: 'SECRET', mfaEnabled: false }));
      await expect(authService.enableMfa('user-1', { totpCode: '000000' })).rejects.toThrow('Invalid TOTP code');
    });
  ...
  describe('Account Lockout Security', () => {
    it('should not reveal if account is locked (anti-enumeration)', async () => {
      const pwHash = await bcrypt.hash('Pass1!', 10);
      mockUserRepo.findOne.mockResolvedValue(buildUser({
        isVerified: true, passwordHash: pwHash,
    ...
    it('should reset failed attempts on successful login', async () => {
      const pwHash = await bcrypt.hash('Pass1!', 10);
      mockUserRepo.findOne.mockResolvedValue(buildUser({
        isVerified: true, passwordHash: pwHash, failedAttempts: 3,
      }));

      await authService.login({ email: 'test@example.com', password: 'Pass1!' });
      expect(mockUserRepo.update).toHaveBeenCalledWith('user-1',
  ...
  describe('Verification Code Expiry Security', () => {
    it('should reject expired email verification codes', async () => {
      ...
      await expect(authService.verifyEmail({ email: 'test@example.com', code: '123456' })).rejects.toThrow('expired');
    });

    it('should reject expired password reset codes', async () => {
      mockUserRepo.findOne.mockResolvedValue(buildUser({ passwordHash: '' }));
      mockPasswordResetRepo.find.mockResolvedValue([{
        id: 'pr-1', code: 'hashed', userId: 'user-1',
        expiresAt: new Date(Date.now() - 100000), used: false,
      }]);
      ...
      await expect(authService.resetPassword({
        email: 'test@example.com', code: '123456', password: 'NewPass1!',
