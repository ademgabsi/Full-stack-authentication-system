...
describe('AuthService', () => {
  ...
  beforeEach(async () => {
    ...
    mockUserRepo.create.mockImplementation((dto: any) => ({
      id: 'new-' + Math.random().toString(36).slice(2),
      ...dto,
    }));
    mockUserRepo.save.mockImplementation((entity: any) =>
      Promise.resolve(entity),
    );
    mockEmailVerificationRepo.create.mockImplementation((dto: any) => dto);
    mockEmailVerificationRepo.save.mockImplementation((entity: any) =>
      Promise.resolve(entity),
    );
    mockPasswordResetRepo.create.mockImplementation((dto: any) => dto);
    mockPasswordResetRepo.save.mockImplementation((entity: any) =>
      Promise.resolve({ id: 'pr-1', ...entity }),
    );
    mockRefreshTokenRepo.create.mockImplementation((dto: any) => dto);
    mockRefreshTokenRepo.save.mockImplementation((entity: any) =>
      Promise.resolve({ id: 'rt-1', ...entity }),
    );
    mockJwtService.sign.mockReturnValue('jwt-token');
    mockDeviceFingerprintService.generateFingerprintHash.mockReturnValue(
      'test-fingerprint-hash',
    );
    mockDeviceFingerprintService.getOrCreateFingerprint.mockResolvedValue({
      isNew: false,
      fingerprint: { id: 'fp-1' },
    });
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: StepUpChallengeService,
          useValue: mockStepUpChallengeService,
        },
      ],
    }).compile();
    ...
    emailVerificationRepo = module.get(
  ...
  describe('enableMfa', () => {
    ...
    it('should throw with invalid TOTP code', async () => {
      ...
      await expect(service.enableMfa('user-1', dto)).rejects.toThrow(
        'Invalid TOTP code',
      );
    });
  ...
  describe('disableMfa', () => {
    ...
    it('should throw with invalid password', async () => {
      ...
      await expect(
        service.disableMfa('user-1', { ...dto, password: 'WrongPass1!' }),
  ...
  describe('refreshTokens', () => {
    it('should refresh token successfully', async () => {
      ...
      mockDataSource.transaction.mockImplementation(async (_level, cb) =>
        cb(fakeTxManager),
      );
  ...
  describe('revokeSession', () => {
    ...
    it('should throw if session not found', async () => {
      mockRefreshTokenRepo.findOne.mockResolvedValue(null);
      await expect(
        service.revokeSession('rt-nonexistent', 'user-1'),
  ...
  describe('revokeAllSessions', () => {
    it('should revoke all sessions except current', async () => {
      ...
      expect(mockRefreshTokenRepo.update).toHaveBeenCalledWith(['other-rt'], {
        isRevoked: true,
      });
      expect(result.message).toContain('revoked');
    });
  ...
  describe('verifyMfa', () => {
    ...
    it('should throw with invalid temp token', async () => {
      ...
      await expect(
        service.verifyMfa({ tempToken: 'bad', totpCode: '123456' }),
    ...
    it('should throw if mfaPending flag is missing', async () => {
      ...
      await expect(
        service.verifyMfa({
          tempToken: 'valid-but-not-mfa',
          totpCode: '123456',
        }),
      ).rejects.toThrow('Invalid token for MFA verification');
    });
    ...
    it('should throw with invalid TOTP code', async () => {
      ...
      await expect(
        service.verifyMfa({ tempToken: 'temp', totpCode: '000000' }),
  ...
  describe('googleOAuthLogin', () => {
    ...
    it('should throw for account conflict', async () => {
      ...
      await expect(service.googleOAuthLogin(googleProfile)).rejects.toThrow(
